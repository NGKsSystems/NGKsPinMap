...
class PinMapApp {
    ...
    setupEventListeners() {
        ...
        // Country change and search
        document.getElementById('country').addEventListener('change', (e) => this.updateStates(e));
        document.getElementById('country-search').addEventListener('input', (e) => this.filterDropdown(e, 'country'));

        // State change and search
        document.getElementById('state').addEventListener('change', (e) => this.updateCities(e));
        document.getElementById('state-search').addEventListener('input', (e) => this.filterDropdown(e, 'state'));

        // City search
        document.getElementById('city-search').addEventListener('input', (e) => this.filterDropdown(e, 'city'));
    ...
    populateCountries() {
        ...
        Object.entries(COUNTRIES_LIST).forEach(([code, data]) => {
            ...
            option.dataset.search = data.name.toLowerCase();
            countrySelect.appendChild(option);
        });
    }

    filterDropdown(e, type) {
        ...
        options.forEach(option => {
            ...
            const isFirst = option.value === '';
            
            if (isFirst || text.includes(searchText)) {
                option.style.display = '';
                if (!isFirst) visibleCount++;
            } else {
        ...
        if (visibleCount === 1) {
            const visibleOption = Array.from(options).find(o => o.style.display !== 'none' && o.value !== '');
            if (visibleOption) {
    ...
    resetForm() {
        ...
        document.getElementById('state').innerHTML = '<option value="">Select a state/region...</option>';
