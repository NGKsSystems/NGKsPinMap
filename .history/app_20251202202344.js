...
class PinMapApp {
    ...
    geocodeLocation(state, city, country, foreignCity, callback) {
        ...
        if (country === 'US') {
            ...
            // For foreign countries, look up from country-specific database (case-insensitive)
            if (WORLD_CITIES_DATABASE[country]) {
                const countryDB = WORLD_CITIES_DATABASE[country];
                const cityKey = Object.keys(countryDB).find(
                    key => key.toLowerCase() === foreignCity.toLowerCase()
                );
                coord = cityKey ? countryDB[cityKey] : [20, 0]; // Default to equator if not found
            } else {
                coord = [20, 0]; // Default to equator if country not in database
            }
        }
