...
class PinMapApp {
    ...
    setupEventListeners() {
        ...
        // Phone toggle in form
        document.getElementById('phone-toggle-form').addEventListener('click', (e) => {
            e.preventDefault();
            const phoneInput = document.getElementById('phone');
            const toggleBtn = document.getElementById('phone-toggle-form');
            
            if (phoneInput.type === 'password') {
                phoneInput.type = 'text';
                toggleBtn.textContent = '🙈';
                toggleBtn.title = 'Hide phone';
            } else {
                phoneInput.type = 'password';
                toggleBtn.textContent = '👁️';
                toggleBtn.title = 'Show phone';
            }
        });
    }
    ...
    resetForm() {
        ...
        // Clear phone data attribute
        document.getElementById('phone').dataset.actualPhone = '';
    ...
    handleFormSubmit(e) {
        ...
        // Get actual phone number (may be masked in form, stored in data attribute)
        const phoneInput = document.getElementById('phone');
        const phone = phoneInput.dataset.actualPhone || phoneInput.value.trim();
        ...
        if (this.editingPinId) {
            // Update existing pin
            this.updatePin(this.editingPinId, name, username, country, state, city, phone, notes, foreignCity);
            this.editingPinId = null;
        } else {
            // Add new pin
    ...
    updatePin(pinId, name, company, country, state, city, email, phone, foreignCity = '') {
        ...
        pin.email = email;
        pin.phone = phone;
        pin.foreignCity = foreignCity;
        pin.tier = this.selectedTier;
        ...
        // Re-geocode and add updated marker
        this.geocodeLocation(state, city, country, foreignCity, (lat, lng) => {
            pin.lat = lat;
            pin.lng = lng;
            this.addMarker(pin);
    ...
    addMarker(pin) {
        ...
        marker.bindPopup(`
            <div class="pin-popup">
                ...
                ${pin.company ? `User Name: ${pin.company}<br>` : ''}
                ${pin.country === 'US' ? `${pin.city}, ${pin.state}` : pin.foreignCity}<br>
                <span class="tier-badge tier-${pin.tier}">Tier: ${pin.tier}</span><br>
                ${pin.phone ? `Phone: <span class="phone-hidden" id="phone-${pin.id}">••••••••</span> <button class="phone-toggle-btn" id="phone-toggle-${pin.id}">Show</button><br>` : ''}
                Lat: ${pin.lat.toFixed(4)}, Lng: ${pin.lng.toFixed(4)}
                <div class="pin-popup-actions">
                    <button class="pin-edit-btn" id="edit-pin-${pin.id}">Edit</button>
                    <button class="pin-delete-btn" id="delete-pin-${pin.id}">Delete</button>
                </div>
            </div>
        `);
        ...
        marker.on('popupopen', () => {
            setTimeout(() => {
                ...
                const phoneToggleBtn = document.getElementById(`phone-toggle-${pin.id}`);
                const phoneDisplay = document.getElementById(`phone-${pin.id}`);
                
                if (editBtn) {
                    editBtn.onclick = () => this.editPin(pin);
                }
                if (deleteBtn) {
                    deleteBtn.onclick = () => this.deletePin(pin.id);
                }
                if (phoneToggleBtn && phoneDisplay) {
                    let isHidden = true;
                    phoneToggleBtn.onclick = () => {
                        if (isHidden) {
                            phoneDisplay.textContent = pin.phone;
                            phoneDisplay.classList.remove('phone-hidden');
                            phoneToggleBtn.textContent = 'Hide';
                            isHidden = false;
                        } else {
                            phoneDisplay.textContent = '••••••••';
                            phoneDisplay.classList.add('phone-hidden');
                            phoneToggleBtn.textContent = 'Show';
                            isHidden = true;
                        }
                    };
                }
    ...
    editPin(pin) {
        ...
        // Set country - get the country name from COUNTRIES_LIST
        const countryData = COUNTRIES_LIST[pin.country];
        if (countryData) {
        ...
        if (pin.country === 'US') {
            // Show state and city, hide foreign city
            document.getElementById('state-wrapper').style.display = 'block';
            document.getElementById('city-wrapper').style.display = 'block';
            document.querySelector('label[for="location-notes"]').style.display = 'none';
            document.getElementById('location-notes').style.display = 'none';
            
            // Set state
            const stateData = COUNTRIES_LIST['US'].states.find(s => s === pin.state);
            if (stateData) {
                document.getElementById('state').value = stateData;
                document.getElementById('state').dataset.value = stateData;
            }
            ...
            // Show foreign city, hide state and city
            document.getElementById('state-wrapper').style.display = 'none';
            document.getElementById('city-wrapper').style.display = 'none';
            document.querySelector('label[for="location-notes"]').style.display = 'block';
            document.getElementById('location-notes').style.display = 'block';
            
            document.getElementById('location-notes').value = pin.foreignCity;
        }
        
        // Mask phone number in form - store actual number in data attribute (invisible to user)
        const phoneInput = document.getElementById('phone');
        // DO NOT populate phone field - keep it blank for privacy
        // Store actual phone only in memory for when editing
        phoneInput.dataset.actualPhone = pin.phone || '';
    ...
    savePins() {
        localStorage.setItem('ngks_pins', JSON.stringify(this.pins));
    }

    loadPins() {
        const saved = localStorage.getItem('ngks_pins');
        if (saved) {
            try {
